...
from habitat_modelling.methods.latent_mapping.models.latent_dense import DenseLatentModel, DropLatentModel2, DropLatentModel3
...
def main(cfg):
    ...
    if cfg['model']['type'] == "dense":
        latent_model = DenseLatentModel(**cfg['model']['params'])
    elif cfg['model']['type'] == "drop":
        if len(cfg['model']['params']['layer_neurons']) == 2:
            latent_model = DropLatentModel2(**cfg['model']['params'])
        elif len(cfg['model']['params']['layer_neurons']) == 3:
            latent_model = DropLatentModel3(**cfg['model']['params'])
        else:
            raise ValueError("Model depth not supported")
    else:
        raise ValueError("Model type not selected")
    latent_model.dump_params(os.path.join(model_dir, 'model_params.json'))
    ...
    nll_loss = torch.nn.NLLLoss(weight=class_weights)
    ...
    if cfg['training']['optimizer']['type'] == 'adam':
        optimizer = torch.optim.Adam(latent_model.parameters(), **get_dict_arguments(cfg['training']['optimizer']))
    elif cfg['training']['optimizer']['type'] == 'sgd':
        optimizer = torch.optim.SGD(latent_model.parameters(), **get_dict_arguments(cfg['training']['optimizer']))
    else:
        raise ValueError("OPtimizer not selected")
    ...
    for epoch in range(cfg['training']['epochs']):
        ...
        latent_model.enable_dropout(True)
        ...
        for batch, data_batch in enumerate(dataloader):
            ...
            y_pred = latent_model(latent_t)

            # loss = cat_loss(y_pred, ytrue_label)
            loss = nll_loss(y_pred, ytrue_label)

            loss.backward()

            optimizer.step()

            yt_cats = np.argmax(y_true.detach().cpu().numpy(), axis=-1)
            yp_cats = np.argmax(y_pred.detach().cpu().numpy(), axis=-1)
            ...
            with open(os.path.join(log_dir, 'training_logs.csv'), 'a') as f:
                writer = csv.writer(f)
                writer.writerow([epoch, batch, loss.item(), batch_acc])
        ...
        if epoch % cfg['training'].get('save_model_freq', 1) == 0:
            # Save state dict
            torch.save(latent_model.state_dict(), os.path.join(model_dir, 'latent_model_%d.pt' % epoch))
            torch.save(optimizer.state_dict(), os.path.join(model_dir, 'optimizer_%d' %epoch))
        ...
        if val_dataset is not None:
            ...
            latent_model.enable_dropout(False)
            ...
            for batch, data_batch in enumerate(val_dataloader):
                ...
                with torch.no_grad():
                    ...
                    y_pred = latent_model(latent)
                    val_loss = nll_loss(y_pred, ytrue_label)
